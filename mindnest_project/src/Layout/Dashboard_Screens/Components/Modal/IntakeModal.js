...
const IntakeModal = ({ show, close }) => {
  ...
  const [classMap, setClassMap] = useState(
    slideContent.map((_, index) => (index === 0 ? "active" : "slided"))
  );
  ...
  const goToNextSlide = (targetIndex) => {
    if (targetIndex >= 0 && targetIndex < slideContent.length) {
      const updatedClassMap = [...classMap];

      // Update current slide: active -> move
      updatedClassMap[currentSlideIndex] = "move";

      // Update next slide: slided -> active
      updatedClassMap[targetIndex] = "active";

      setClassMap(updatedClassMap);
      
    // Wait for animation to finish before advancing index
      setTimeout(() => {
    ...
    >
      <div className="modal-wrapper">
        {slideContent.map((slide, index) => (
          <div
            key={slide.id}
            className={`slide-pop ${classMap[index]}`}
          >
            ...
            <ModalBody>
              <div className="modal-body-inner intake-modal">
                {currentSlideIndex.discriptionHeading && (
                  <div className="intake-heading">
                ...
                {slide.discriptionParagraph && <p>{currentSlide.discriptionParagraph}</p>}

              </div>
      
              {currentSlideIndex === 2
    ? currentSlide.renderBody && currentSlide.renderBody(key, setKey, checkedItems, handleCheckboxChange, goToNextSlide)
    : currentSlide.renderBody && currentSlide.renderBody(handleSubmit, goToNextSlide)}
            </ModalBody>
